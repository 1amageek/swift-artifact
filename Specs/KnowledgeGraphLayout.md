...
| 制約 | 意味 | 初期値 |
|---|---|---:|
| Edge-Edge | Edge 同士の最小表示距離。共有 endpoint を持たない Edge の重なりを防ぐ | 14 pt |
...
| 5 | Edge-Edge 視認性 | Edge 長と角数が同程度の場合だけ、既存 Edge との距離が広い候補を優先 |
| 6 | 多角迂回 | Node を避けるために必要な場合のみ角を増やす |
...
3. Edge 長が短い
4. 角数が少ない
5. 既存 Edge との Edge-Edge 距離を満たしている
