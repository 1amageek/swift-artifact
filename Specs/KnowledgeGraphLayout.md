...
共有していない endpoint を中央からずらしてはならない。
...
| 6 | Edge-Edge 視認性 | Edge 長と角数が同程度の場合だけ、既存 Edge との距離が広い候補を優先 |
| 7 | 多角迂回 | Node を避けるために必要な場合のみ角を増やす |
...
ただし、swap によって対象 Edge 群の合計 route 長が伸びる候補は採用しない。
route 長が同等の場合だけ、Edge 同士の交差数が少ない割り当てを優先する。
この swap 評価は、変更される Edge と近傍 segment だけを比較する差分評価で行う。
...
| port swap 差分評価 | route 長を伸ばさず交差だけを減らす | 変更 Edge と近傍 segment だけを評価し、最良改善を採用すること |
| 同長関節 lane 正規化 | H-V-H / V-H-V の関節を同じ route 長の範囲でスライドする | 法線接続、Node 非干渉、Edge-Edge 距離を保ったまま 14pt lane rhythm に揃えること |

```text
port candidates -> shortest valid route -> nearby collision check -> local port swap -> equal-length lane normalize
       │                    │                         │                    │
       └──── keeps center   └──── length first        └──── exact result   └──── no length growth
```
