...
struct KnowledgeGraphLayout: Sendable {
    ...
    private static func expandedGroupBoxes(
        ...
        return expandedGroupBoxes(
            ...
            sizes: sizes,
            groups: groups,
            groupMembers: membership.groupMembers,
            groupMemberSets: membership.groupMembers.map(Set.init),
    ...
    private static func expandedGroupBoxes(
        ...
        for (groupIndex, group) in groups.enumerated()
            ...
            let indices = groupMembers[groupIndex]
            guard !indices.isEmpty else { continue }
            var rect = CGRect.null
            for index in indices {
            ...
            result.append(LayeredGroupBox(
                groupIndex: groupIndex,
                indices: indices,
                memberSet: groupMemberSets[groupIndex],
                rect: padded
            ))
        }
    ...
    private static func packGroupContents(
        ...
        let childrenByParent = directNestedChildrenByParent(memberSets: memberSets)
        let parentOrder = groups.indices.sorted { lhs, rhs in
            let leftCount = memberSets[lhs].count
            let rightCount = memberSets[rhs].count
            if leftCount != rightCount {
                return leftCount < rightCount
            }
            return lhs < rhs
        ...
        for _ in 0..<iterations {
            var boxesByGroupIndex = groupBoxesByIndex(
                positions: positions,
                sizes: sizes,
                groups: groups,
                indexByID: indexByID
            ...
            for parentIndex in parentOrder {
                guard groupMembershipIsLaminar(parentIndex, memberSets: memberSets) else { continue }
                let childIndices = childrenByParent[parentIndex] ?? []
                guard nestedChildMemberSetsAreDisjoint(childIndices, memberSets: memberSets) else { continue }
                let units = groupContentPackingUnits(
                    parentIndex: parentIndex,
                    childIndices: childIndices,
                    memberSets: memberSets,
                    boxesByGroupIndex: boxesByGroupIndex,
                    positions: positions,
                    sizes: sizes
                )
                guard units.count > 1 else { continue }
                let hasDirectNodeUnit = units.contains { $0.groupIndices.isEmpty }
                let before = outlineRect(for: units)
                if childIndices.isEmpty {
                    guard hasDirectNodeUnit,
                          groupHasNoInternalEdges(memberSet: memberSets[parentIndex], edges: edges, indexByID: indexByID)
                    else { continue }
                    let candidate = verticallyCompactedDirectNodePositions(
                        positions: positions,
                        units: units,
                        gap: LayoutSpacing.groupInternalNodeNodeVertical,
                        anchor: before.origin
                    )
                    let area = layoutArea(movedOutlineRect(for: units, from: positions, to: candidate))
                    guard area < layoutArea(before) - 0.5 else { continue }
                    positions = candidate
                    boxesByGroupIndex = groupBoxesByIndex(
                        positions: positions,
                        sizes: sizes,
                        groups: groups,
                        indexByID: indexByID
                ...
                let itemSizes = units.map { unit in
                    CGSize(
                        ...
                        height: unit.rect.height + gapSize.height
                    )
                }
                let candidates = anchoredPackedPositionCandidates(
                    positions: positions,
                    units: units,
                    itemSizes: itemSizes,
                    gapSize: gapSize,
                ...
                var bestArea = layoutArea(before)
                ...
                guard bestArea < layoutArea(before) - 0.5 else { continue }
    ...
    private static func groupBoxesByIndex(
        positions: [CGPoint],
        sizes: [CGSize],
        groups: [CompoundGraph.Group],
        indexByID: [CompoundGraph.Card.ID: Int]
    ) -> [Int: LayeredGroupBox] {
        Dictionary(uniqueKeysWithValues: expandedGroupBoxes(
            positions: positions,
            sizes: sizes,
            groups: groups,
            indexByID: indexByID,
    ...
    private static func groupMembershipIsLaminar(
        _ groupIndex: Int,
        memberSets: [Set<Int>]
    ) -> Bool {
        guard memberSets.indices.contains(groupIndex) else { return false }
        let members = memberSets[groupIndex]
        guard !members.isEmpty else { return false }
        for otherIndex in memberSets.indices where otherIndex != groupIndex {
            let otherMembers = memberSets[otherIndex]
            guard !otherMembers.isEmpty else { continue }
            let intersection = members.intersection(otherMembers)
            if !intersection.isEmpty
                && intersection != members
                && intersection != otherMembers {
                return false
            }
        }
        return true
    ...
    private static func groupContentPackingUnits(
        parentIndex: Int,
        childIndices: [Int],
        memberSets: [Set<Int>],
        boxesByGroupIndex: [Int: LayeredGroupBox],
        positions: [CGPoint],
        sizes: [CGSize]
    ) -> [KnowledgeGraphLayoutCompactionUnit] {
        guard memberSets.indices.contains(parentIndex) else { return [] }
        var units: [KnowledgeGraphLayoutCompactionUnit] = []
        units.reserveCapacity(childIndices.count + memberSets[parentIndex].count)
        var nestedMembers: Set<Int> = []

        for childIndex in childIndices.sorted() {
            guard let box = boxesByGroupIndex[childIndex] else { continue }
            units.append(KnowledgeGraphLayoutCompactionUnit(
                groupIndices: [childIndex],
                indices: box.memberSet.sorted(),
                memberSet: box.memberSet,
                rect: box.rect
            ))
            nestedMembers.formUnion(box.memberSet)
        }

        let directMembers = memberSets[parentIndex]
            .subtracting(nestedMembers)
            .sorted()
        for index in directMembers {
            let rect = CGRect(
    ...
    private static func groupHasNoInternalEdges(
        memberSet: Set<Int>,
        edges: [CompoundGraph.CardEdge],
        indexByID: [CompoundGraph.Card.ID: Int]
    ) -> Bool {
        for edge in edges {
            ...
            if memberSet.contains(sourceIndex) && memberSet.contains(targetIndex) {
                return false
            }
        }
        return true
    ...
    private static func verticallyCompactedDirectNodePositions(
        positions: [CGPoint],
        units: [KnowledgeGraphLayoutCompactionUnit],
        gap: CGFloat,
        anchor: CGPoint
    ) -> [CGPoint] {
        let sortedUnits = units.sorted { lhs, rhs in
            if abs(lhs.rect.minY - rhs.rect.minY) > 0.001 {
        ...
        let width = sortedUnits.map(\.rect.width).max() ?? 0
        let centerX = anchor.x + width / 2
        var cursorY = anchor.y
        var result = positions
        for unit in sortedUnits {
            let dx = centerX - unit.rect.midX
            let dy = cursorY - unit.rect.minY
            for index in unit.indices {
                result[index].x += dx
                result[index].y += dy
            }
            cursorY += unit.rect.height + gap
        }
        return result
    }

    private static func groupContentPackingGapSize(
        units: [KnowledgeGraphLayoutCompactionUnit],
        edges: [CompoundGraph.CardEdge],
        indexByID: [CompoundGraph.Card.ID: Int]
    ) -> CGSize {
        guard units.count > 1 else { return .zero }
        var horizontal: CGFloat = 0
        var vertical: CGFloat = 0
        let edgeCounts = edgeCountByNodePair(edges: edges, indexByID: indexByID)
        for lhs in 0..<(units.count - 1) {
            for rhs in (lhs + 1)..<units.count {
                let gap = groupContentGap(lhs: units[lhs], rhs: units[rhs], edgeCounts: edgeCounts)
                horizontal = max(horizontal, gap.width)
                vertical = max(vertical, gap.height)
            }
        }
        return CGSize(width: horizontal, height: vertical)
    }

    private static func groupContentGap(
        lhs: KnowledgeGraphLayoutCompactionUnit,
        rhs: KnowledgeGraphLayoutCompactionUnit,
        edgeCounts: [UInt64: Int]
    ) -> CGSize {
        ...
        let edgeCount = maxEdgeCountBetween(lhs.memberSet, rhs.memberSet, edgeCounts: edgeCounts)
        let gap = minimumNodeNodeGap(edgeMultiplicity: edgeCount, sameGroup: true)
        return CGSize(width: gap.horizontal, height: gap.vertical)
    }

    private static func edgeCountByNodePair(
        edges: [CompoundGraph.CardEdge],
        indexByID: [CompoundGraph.Card.ID: Int]
    ) -> [UInt64: Int] {
        ...
        for edge in edges where edge.source != edge.target {
            ...
            let key = pairKey(sourceIndex, targetIndex)
            result[key, default: 0] += 1
        }
        return result
    }

    private static func maxEdgeCountBetween(
        _ lhs: Set<Int>,
        _ rhs: Set<Int>,
        edgeCounts: [UInt64: Int]
    ) -> Int {
    ...
    private static func anchoredPackedPositionCandidates(
        positions: [CGPoint],
        units: [KnowledgeGraphLayoutCompactionUnit],
        itemSizes: [CGSize],
        gapSize: CGSize,
        ...
        maxRectsPackedPositionCandidates(
            positions: positions,
            units: units,
            itemSizes: itemSizes,
            gapSize: gapSize,
            ...
            let dy = anchor.y - packed.minY
            return translatedPositions(candidate, moving: units.flatMap(\.indices), dx: dx, dy: dy)
        }
    }

    private static func outlineRect(for units: [KnowledgeGraphLayoutCompactionUnit]) -> CGRect {
        units.reduce(CGRect.null) { partial, unit in
            partial.isNull ? unit.rect : partial.union(unit.rect)
        }
    }

    private static func movedOutlineRect(
        for units: [KnowledgeGraphLayoutCompactionUnit],
        from oldPositions: [CGPoint],
        to newPositions: [CGPoint]
    ) -> CGRect {
        units.reduce(CGRect.null) { partial, unit in
            guard let first = unit.indices.first else { return partial }
            let dx = newPositions[first].x - oldPositions[first].x
            let dy = newPositions[first].y - oldPositions[first].y
            let rect = unit.rect.offsetBy(dx: dx, dy: dy)
            return partial.isNull ? rect : partial.union(rect)
        }
    }

    private static func translatedPositions(
        _ positions: [CGPoint],
        moving indices: [Int],
        dx: CGFloat,
        dy: CGFloat
    ) -> [CGPoint] {
        guard abs(dx) > 0.001 || abs(dy) > 0.001 else { return positions }
        var result = positions
    ...
    private static func distanceGroupIndexSet(
        ...
        return distanceGroupIndexSet(groupMemberSets: memberSets)
    }

    private static func distanceGroupIndexSet(groupMemberSets memberSets: [Set<Int>]) -> Set<Int> {
    ...
    private static func graphLayoutCostContext(
        edges: [CompoundGraph.CardEdge],
        groups: [CompoundGraph.Group],
        indexByID: [CompoundGraph.Card.ID: Int]
    ) -> KnowledgeGraphLayoutCostContext {
        var edgeEndpoints: [KnowledgeGraphResolvedCardEdge] = []
        edgeEndpoints.reserveCapacity(edges.count)
        var edgeMultiplicity: [UInt64: Int] = [:]
        edgeMultiplicity.reserveCapacity(edges.count)
        for edge in edges {
            guard
                let source = indexByID[edge.source],
                let target = indexByID[edge.target],
            ...
            edgeEndpoints.append(KnowledgeGraphResolvedCardEdge(source: source, target: target))
            edgeMultiplicity[pairKey(source, target), default: 0] += 1
        }

        let membership = groupMembershipIndex(groups: groups, indexByID: indexByID)
        let groupMemberSets = membership.groupMembers.map(Set.init)
        return KnowledgeGraphLayoutCostContext(
            edgeEndpoints: edgeEndpoints,
            edgeMultiplicity: edgeMultiplicity,
            groupEdgeCounts: disjointGroupEdgeCounts(
                edgeEndpoints: edgeEndpoints,
                groups: groups,
                membership: membership,
                groupMemberSets: groupMemberSets
            )
        )
    }
    ...
    private static func sameGroupPairKeys(
        ...
        sameGroupPairKeys(
            groups: groups,
            membership: groupMembershipIndex(groups: groups, indexByID: indexByID)
        )
    }
    ...
    private static func sameGroupPairKeys(
        ...
        var result: Set<UInt64> = []
        for groupIndex in groups.indices where groups[groupIndex].cohesionStrength > 0 {
            let indices = membership.groupMembers[groupIndex]
            guard indices.count > 1 else { continue }
            for a in 0..<(indices.count - 1) {
                for b in (a + 1)..<indices.count {
    ...
    private static func resolveOverlaps(
        ...
        let usesSameGroupPairs = !sameGroupPairs.isEmpty
        let subpixelPushThreshold: CGFloat = 0.125

        func resolvePair(_ i: Int, _ j: Int, rects: inout [CGRect]) -> CGFloat {
            let isSameGroup = usesSameGroupPairs && sameGroupPairs.contains(pairKey(i, j))
        ...
        for _ in 0..<iterations {
            var maxPush: CGFloat = 0
            var rects = cardRects(positions: positions, sizes: sizes)

            if n <= 24 {
                for i in 0..<(n - 1) {
                    for j in (i + 1)..<n {
                        maxPush = max(maxPush, resolvePair(i, j, rects: &rects))
                    }
                }
                if maxPush == 0 || maxPush < subpixelPushThreshold { break }
                continue
            }
    ...
    private enum PackingTuning {
        ...
        static let maxLargePackedCandidates = 8
        static let maxFreeRects = 96
        static let maxEstimatedConflictRoutes = 120
        static let largeGraphCardCount = 160
        static let regularOutlinePasses = 4
        static let largeGraphOutlinePasses = 1
        static let regularCandidateConstraintIterations = 6
    }

    private enum RoutingTuning {
        static let exactObstacleRoutingNodeCount = 36
        static let maxRequiredObstacleSeeds = 2
        static let maxSupplementalObstacleSeeds = 3
        static let maxObstacleRects = 28
        static let maxDetourAxisValues = 10
        static let maxCompoundDetourAxisValues = 5
        static let maxRouteSearchExpansions = 1_800
        static let maxRouteSearchStates = 8_000
        static let maxFullRouteOptimizationEdges = 180
        static let maxEqualLengthLaneNormalizationEdges = 160
        static let compactCandidateNodeCount = 120
    }

    private enum EdgeLabelTuning {
        static let maxSamplesPerSegment = 9
        static let maxSamplesPerEdge = 48
        static let largeGraphEdgeCount = 80
        static let largeGraphPlacementPasses = 2
        static let regularPlacementPasses = 4
    }
    ...
    private static func minimizeConstrainedOutlineArea(
        ...
        let costContext = graphLayoutCostContext(edges: edges, groups: groups, indexByID: indexByID)
        let edgeMultiplicity = costContext.edgeMultiplicity
        let groupEdgeCounts = costContext.groupEdgeCounts

        let usesLargeGraphTuning = positions.count > PackingTuning.largeGraphCardCount
        let outlinePasses = usesLargeGraphTuning
            ? PackingTuning.largeGraphOutlinePasses
            : PackingTuning.regularOutlinePasses
        let candidateConstraintIterations = usesLargeGraphTuning
            ? 0
            : PackingTuning.regularCandidateConstraintIterations
        ...
        for _ in 0..<outlinePasses {
            ...
            let usePackedOutline = shouldUsePackedOutline(
                units: globalUnits,
                groups: groups,
                indexByID: indexByID
            )
            let units = usePackedOutline
                ? globalUnits
                : rankPreservingLayoutCompactionUnits(
                    positions: positions,
                    sizes: sizes,
                    groups: groups,
                    indexByID: indexByID
            ...
            let oldOutline = usePackedOutline
                ? nil
                : rankPreservingLayoutOutlineRect(
                    positions: positions,
                    sizes: sizes,
                    groups: groups,
                    indexByID: indexByID
                )
            var bestArea = oldOutline.map(layoutArea) ?? 0
            let costModel = usePackedOutline
                ? placementCostModel(
                ...
                : nil
            let oldCost = costModel?.cost(for: positions)
            ...
            for proposed in compactionCandidates {
                ...
                enforceLayoutDistanceConstraints(
                    ...
                    groups: groups,
                    indexByID: indexByID,
                    iterations: candidateConstraintIterations
                )
                if let costModel {
                    let candidateCost = costModel.cost(for: candidate)
                    if bestCost.map({ candidateCost.isBetter(than: $0) }) ?? true {
                        bestCost = candidateCost
                        bestCandidate = candidate
                    }
                } else {
                    let newOutline = rankPreservingLayoutOutlineRect(
                        positions: candidate,
                        sizes: sizes,
                        groups: groups,
                        indexByID: indexByID
            ...
            if usePackedOutline {
                ...
                guard let oldOutline, bestArea < layoutArea(oldOutline) - 0.5 else { break }
            }
    ...
    private static func packAxisAlignedGlobalOutline(
        ...
        guard shouldUsePackedOutline(units: units, groups: groups, indexByID: indexByID) else {
            return false
        }
        ...
        let costModel = placementCostModel(
            basePositions: positions,
            sizes: sizes,
            units: units,
            context: costContext
        ...
        var bestCost = oldCost
        for candidate in candidates {
            let cost = costModel.cost(for: candidate)
    ...
    private static func placementCostModel(
        basePositions: [CGPoint],
        sizes: [CGSize],
        units: [KnowledgeGraphLayoutCompactionUnit],
        context: KnowledgeGraphLayoutCostContext
    ) -> KnowledgeGraphPlacementCostModel {
        KnowledgeGraphPlacementCostModel(
            basePositions: basePositions,
            sizes: sizes,
            units: units,
            unitPairs: placementUnitPairs(
                units: units,
                edgeMultiplicity: context.edgeMultiplicity,
                groupEdgeCounts: context.groupEdgeCounts
            ),
            routeEdges: placementRouteEdges(
                units: units,
                cardCount: basePositions.count,
                context: context
            ),
            spacing: KnowledgeGraphPlacementCostSpacing(
                edgeNode: LayoutSpacing.edgeNode,
                edgeEdgeRoute: LayoutSpacing.edgeEdgeRoute
            ),
            maxEstimatedConflictRoutes: PackingTuning.maxEstimatedConflictRoutes
        )
    }

    private static func placementUnitPairs(
        units: [KnowledgeGraphLayoutCompactionUnit],
        edgeMultiplicity: [UInt64: Int],
        groupEdgeCounts: [UInt64: Int]
    ) -> [KnowledgeGraphUnitPlacementPair] {
        var pairs: [KnowledgeGraphUnitPlacementPair] = []
        pairs.reserveCapacity(max(0, units.count * (units.count - 1) / 2))
        if units.count > 1 {
            for lhs in 0..<(units.count - 1) {
                for rhs in (lhs + 1)..<units.count where units[lhs].memberSet.isDisjoint(with: units[rhs].memberSet) {
                    pairs.append(KnowledgeGraphUnitPlacementPair(
                        lhs: lhs,
                        rhs: rhs,
                        gap: minimumUnitGap(
                            lhs: units[lhs],
    ...
    private static func placementRouteEdges(
        units: [KnowledgeGraphLayoutCompactionUnit],
        cardCount: Int,
        context: KnowledgeGraphLayoutCostContext
    ) -> [KnowledgeGraphUnitPlacementRouteEdge] {
        let cardToUnit = cardUnitIndex(units: units, cardCount: cardCount)
        var routeMultiplicity: [UInt64: (sourceUnit: Int, targetUnit: Int, count: Int)] = [:]
        routeMultiplicity.reserveCapacity(context.edgeEndpoints.count)
        for edge in context.edgeEndpoints {
            guard
                let sourceUnit = cardToUnit[edge.source],
                let targetUnit = cardToUnit[edge.target],
                sourceUnit != targetUnit
            else { continue }
            let key = pairKey(sourceUnit, targetUnit)
            if var existing = routeMultiplicity[key] {
                existing.count += 1
                routeMultiplicity[key] = existing
            } else {
                routeMultiplicity[key] = (sourceUnit, targetUnit, 1)
            }
        }
        return routeMultiplicity.values
            .map { KnowledgeGraphUnitPlacementRouteEdge(
                sourceUnit: $0.sourceUnit,
                targetUnit: $0.targetUnit,
                multiplicity: $0.count
            ) }
            .sorted { lhs, rhs in
                if lhs.sourceUnit != rhs.sourceUnit {
                    return lhs.sourceUnit < rhs.sourceUnit
                }
                return lhs.targetUnit < rhs.targetUnit
            }
    }
    ...
    private static func rankPreservingLayoutCompactionUnits(
        positions: [CGPoint],
        sizes: [CGSize],
        groups: [CompoundGraph.Group],
        indexByID: [CompoundGraph.Card.ID: Int]
    ...
    private static func shouldUsePackedOutline(
        units: [KnowledgeGraphLayoutCompactionUnit],
        groups: [CompoundGraph.Group],
        indexByID: [CompoundGraph.Card.ID: Int]
    ) -> Bool {
    ...
    private static func outlinePackingOrders(
        _ units: [KnowledgeGraphLayoutCompactionUnit],
        edgeMultiplicity: [UInt64: Int] = [:]
    ) -> [[Int]] {
    ...
    private static func edgeAwareOutlinePackingOrders(
        ...
        let indices = Array(units.indices)
        let connections = unitConnectionMatrix(units: units, edgeMultiplicity: edgeMultiplicity)
        let degrees = indices.map { unitIndex in
            indices.reduce(0) { partial, otherIndex in
                guard unitIndex != otherIndex else { return partial }
                return partial + connections[unitIndex, otherIndex]
            }
        }
    ...
    private struct UnitConnectionMatrix {
        let count: Int
        private let values: [Int]

        init(count: Int, values: [Int]) {
            self.count = count
            self.values = values
        }

        subscript(lhs: Int, rhs: Int) -> Int {
            values[lhs * count + rhs]
        }
    }

    private static func unitConnectionMatrix(
        units: [KnowledgeGraphLayoutCompactionUnit],
        edgeMultiplicity: [UInt64: Int]
    ) -> UnitConnectionMatrix {
        let count = units.count
        var values = Array(repeating: 0, count: count * count)
        guard count > 1 else {
            return UnitConnectionMatrix(count: count, values: values)
        }
        for lhs in 0..<(count - 1) {
            for rhs in (lhs + 1)..<count {
                let edgeCount = edgeCountBetweenUnits(
                    units[lhs],
                    units[rhs],
                    edgeMultiplicity: edgeMultiplicity
                )
                values[lhs * count + rhs] = edgeCount
                values[rhs * count + lhs] = edgeCount
            }
        }
        return UnitConnectionMatrix(count: count, values: values)
    }
    ...
    private static func edgeAwareOutlinePackingOrder(
        _ units: [KnowledgeGraphLayoutCompactionUnit],
        connections: UnitConnectionMatrix,
        start: Int,
        degrees: [Int]
    ) -> [Int] {
        var order = [start]
        ...
        while let next = remaining.max(by: { lhs, rhs in
            let leftConnection = order.reduce(0) { partial, placed in
                partial + connections[lhs, placed]
            }
            let rightConnection = order.reduce(0) { partial, placed in
                partial + connections[rhs, placed]
            }
    ...
    private static func maxRectsPackedPositionCandidates(
        ...
        guard units.count > 1 else { return [] }
        let boundedOrders = Array(orders.prefix(PackingTuning.maxPackingOrders))
        let widths = maxRectsCandidateWidths(itemSizes: itemSizes, orders: boundedOrders)
        guard !boundedOrders.isEmpty, !widths.isEmpty else { return [] }
        if units.count > PackingTuning.maxExactMaxRectsUnits {
            return shelfPackedPositionCandidates(
                positions: positions,
                units: units,
                itemSizes: itemSizes,
                gapSize: gapSize,
        ...
        let candidateLimit = units.count > PackingTuning.maxExactMaxRectsUnits
            ? PackingTuning.maxLargePackedCandidates
            : PackingTuning.maxPackedCandidates
        return candidates
            .sorted { lhs, rhs in
                lhs.area < rhs.area
            }
    ...
    private static func maxRectsCandidateWidths(itemSizes: [CGSize], orders: [[Int]]) -> [CGFloat] {
        ...
        let allWidths = keys.sorted().map(CGFloat.init)
        guard allWidths.count > PackingTuning.maxPackingWidths else { return allWidths }
        let targetWidth = sqrt(totalArea * CGFloat(1.5))
        var selected: [CGFloat] = []

        func appendUnique(_ value: CGFloat) {
            guard !selected.contains(where: { abs($0 - value) < 0.5 }) else { return }
            selected.append(value)
        }

        appendUnique(ceil(minWidth))
        for width in allWidths.sorted(by: { abs($0 - targetWidth) < abs($1 - targetWidth) }) {
            appendUnique(width)
            if selected.count >= PackingTuning.maxPackingWidths - 1 { break }
        }
        appendUnique(ceil(maxWidth))
        return selected.sorted()
    }

    private static func shelfPackedPositionCandidates(
        positions: [CGPoint],
        units: [KnowledgeGraphLayoutCompactionUnit],
        itemSizes: [CGSize],
        gapSize: CGSize,
        ...
        var candidates: [MaxRectsPacking] = []
        candidates.reserveCapacity(orders.count * widths.count)
        for order in orders {
            for width in widths {
                guard let packing = shelfPack(
                    positions: positions,
                    units: units,
                    itemSizes: itemSizes,
                    order: order,
        ...
        return candidates
            .sorted { lhs, rhs in
                ...
                return lhs.positions.lexicographicallyPrecedes(rhs.positions) { lhsPoint, rhsPoint in
    ...
    private static func shelfPack(
        ...
        guard order.count == units.count else { return nil }
        ...
        for unitIndex in order {
            ...
            guard size.width <= width + 0.001 else { return nil }
            if cursor.x > 0, cursor.x + size.width > width + 0.001 {
                cursor.x = 0
                cursor.y += rowHeight
                rowHeight = 0
            }
            placedInflated[unitIndex] = CGRect(origin: cursor, size: size)
            cursor.x += size.width
    ...
    private static func packedPositions(
        basePositions: [CGPoint],
        units: [KnowledgeGraphLayoutCompactionUnit],
        placedInflated: [CGRect],
        gapSize: CGSize
    ) -> MaxRectsPacking {
        let origins = placedInflated.enumerated().map { unitIndex, rect in
            CGPoint(
                x: rect.minX + gapSize.width / 2 - units[unitIndex].rect.minX,
                y: rect.minY + gapSize.height / 2 - units[unitIndex].rect.minY
            )
        }
        var result = basePositions
        for (unitIndex, unit) in units.enumerated() {
        ...
        let outline = units.indices.reduce(CGRect.null) { partial, unitIndex in
            ...
            let rect = CGRect(
                x: inflated.minX + gapSize.width / 2,
                y: inflated.minY + gapSize.height / 2,
    ...
    private static func maxRectsPack(
        ...
        for unitIndex in order {
            ...
            if freeRects.count > PackingTuning.maxFreeRects {
                freeRects = rankedMaxRects(freeRects, limit: PackingTuning.maxFreeRects)
            }
        }
    ...
    private static func rankedMaxRects(_ rects: [CGRect], limit: Int) -> [CGRect] {
        rects
            .sorted { lhs, rhs in
                let leftArea = layoutArea(lhs)
                let rightArea = layoutArea(rhs)
                if abs(leftArea - rightArea) > 0.5 {
                    return leftArea > rightArea
                }
    ...
    private static func edgeCountBetweenUnits(
        _ lhs: KnowledgeGraphLayoutCompactionUnit,
        _ rhs: KnowledgeGraphLayoutCompactionUnit,
        edgeMultiplicity: [UInt64: Int]
    ) -> Int {
        var count = 0
    ...
    private static func minimumNodeNodeGap(edgeMultiplicity: Int, sameGroup: Bool) -> KnowledgeGraphNodeNodeGap {
        ...
        guard edgeMultiplicity > 0 else {
            return KnowledgeGraphNodeNodeGap(horizontal: baseHorizontal, vertical: baseVertical)
        }
    ...
    private static func disjointGroupEdgeCounts(
        ...
        return disjointGroupEdgeCounts(
            edgeEndpoints: edgeEndpoints,
            groups: groups,
            membership: membership,
            groupMemberSets: groupMemberSets
    ...
    private static func disjointGroupEdgeCounts(
        edgeEndpoints: [KnowledgeGraphResolvedCardEdge],
        groups: [CompoundGraph.Group],
        membership: GroupMembershipIndex,
        groupMemberSets: [Set<Int>]
    ) -> [UInt64: Int] {
        var result: [UInt64: Int] = [:]
        result.reserveCapacity(edgeEndpoints.count)
        for edge in edgeEndpoints {
            guard
                let sourceGroups = membership.memberToGroups[edge.source],
                let targetGroups = membership.memberToGroups[edge.target]
            else { continue }
    ...
    private static func rankPreservingLayoutOutlineRect(
        positions: [CGPoint],
        sizes: [CGSize],
        groups: [CompoundGraph.Group],
        indexByID: [CompoundGraph.Card.ID: Int]
    ...
    private struct RouteNodeIndex {
        ...
        func segmentIntersectsNode(
            ...
            excludingFirst first: Int,
            excludingSecond second: Int
        ) -> Bool {
            return index.contains(in: segmentBounds(start, end)) { node in
                node.index != first
                    && node.index != second
    ...
    private static func computeEdgeRoutes(
        ...
        guard edges.count <= RoutingTuning.maxFullRouteOptimizationEdges else {
            return routes
        }
        ...
        guard edges.count <= RoutingTuning.maxEqualLengthLaneNormalizationEdges else {
            return routesBeforeNormalization
        }
    ...
    private struct SharedEndpointFlags {
        let lhsSource: Bool
        let lhsTarget: Bool
        let rhsSource: Bool
        let rhsTarget: Bool
        ...
        var isEmpty: Bool {
            !(lhsSource || lhsTarget || rhsSource || rhsTarget)
        }
    }

    private struct RouteConflictContext {
        let score: KnowledgeGraphRouteConflictScore
    ...
    private struct PortSwapImprovement {
        ...
        let conflictBefore: KnowledgeGraphRouteConflictScore
        let conflictAfter: KnowledgeGraphRouteConflictScore
    ...
    private struct PortAssignmentScore {
        ...
        let conflict: KnowledgeGraphRouteConflictScore
    ...
    private static func affectedEdges(
        lhs: EdgeIdentifier,
        rhs: EdgeIdentifier,
        edgeByID: [EdgeIdentifier: CompoundGraph.CardEdge]
    ) -> [CompoundGraph.CardEdge] {
        if lhs == rhs {
            return edgeByID[lhs].map { [$0] } ?? []
        }
        var result: [CompoundGraph.CardEdge] = []
        result.reserveCapacity(2)
        if let edge = edgeByID[lhs] {
            result.append(edge)
        }
        if let edge = edgeByID[rhs] {
            result.append(edge)
        }
        return result
    }
    ...
    private static func optimizeSharedPortCrossings(
        ...
        for _ in 0..<2 {
            ...
            let fullRouteSegmentIndex = routeSegmentIndex(edges: edges, routes: optimized)
            ...
            for bucket in sortedEndpointBuckets(buckets.keys) {
                ...
                for lhsIndex in 0..<(sortedEntries.count - 1) {
                    ...
                    let currentLocalConflict = localRouteConflictScore(
                        routes: optimized,
                        affectedEdges: affectedEdgeList,
                        baseRouteSegmentIndex: fullRouteSegmentIndex,
                        ignoringIndexedEdgeIDs: affectedEdgeIDs
                    )
    ...
    private static func straightenMovableDirectRoutes(
        ...
        for _ in 0..<4 {
            ...
            for edge in edges where edge.source != edge.target {
                ...
                let currentConflict = localRouteConflictScore(
                    routes: optimized,
                    affectedEdges: [edge],
                    baseRouteSegmentIndex: fullRouteSegmentIndex
                )
                for directPoints in directRouteWithPortBiasCandidates(
                    ...
                    let conflictAfter = localRouteConflictScore(
                    ...
                    let improvement = DirectRouteAlignmentImprovement(
                        ...
                        conflictBefore: currentConflict,
	    ...
	    private static func routesBySwappingEndpointPorts(
	        ...
	        indexByID: [CompoundGraph.Card.ID: Int],
	        nodeIndex: RouteNodeIndex,
	        affectedEdges: [CompoundGraph.CardEdge],
	        baseRouteSegmentIndex: RouteSegmentIndex,
	        routeCandidateCache: inout FixedEndpointRouteCandidateCache
	    ) -> [EdgeIdentifier: EdgeRoute]? {
        var swappedLHS = lhs
        var swappedRHS = rhs
        ...
        func endpoint(for key: EdgeEndpointKey) -> RouteEndpoint? {
            if key == lhs.key { return swappedLHS }
            if key == rhs.key { return swappedRHS }
        ...
        for edge in affectedEdges {
            guard
                let sourceIndex = indexByID[edge.source],
                let targetIndex = indexByID[edge.target],
                let sourceEndpoint = endpoint(for: EdgeEndpointKey(edgeID: edge.id, isSource: true)),
                let targetEndpoint = endpoint(for: EdgeEndpointKey(edgeID: edge.id, isSource: false))
            else { return nil }
    ...
    private static func portAssignmentScore(
        ...
        affectedEdges: [CompoundGraph.CardEdge],
        baseRouteSegmentIndex: RouteSegmentIndex,
        ignoringIndexedEdgeIDs: Set<EdgeIdentifier> = []
    ) -> PortAssignmentScore {
        PortAssignmentScore(
            corners: routeCornerSum(routes: routes, affectedEdges: affectedEdges),
            length: routeLengthSum(routes: routes, affectedEdges: affectedEdges),
            maximumLength: routeMaximumLength(routes: routes, affectedEdges: affectedEdges),
            conflict: localRouteConflictScore(
                routes: routes,
                affectedEdges: affectedEdges,
                baseRouteSegmentIndex: baseRouteSegmentIndex,
                ignoringIndexedEdgeIDs: ignoringIndexedEdgeIDs
            )
        )
    }
    ...
    private static func routeConflictContext(
        ...
        guard edges.count > 1 else {
            return RouteConflictContext(
                score: KnowledgeGraphRouteConflictScore(crossings: 0, clearancePenalty: 0),
                edgeIDs: []
            )
        }
        ...
        for routedEdge in routedEdges {
            ...
            let currentEdgeKey = edgeSortKey(routedEdge.edge.id)
            for offset in 1..<routedEdge.points.count {
        ...
        return RouteConflictContext(
            score: KnowledgeGraphRouteConflictScore(crossings: crossings, clearancePenalty: clearancePenalty),
            edgeIDs: edgeIDs
        )
    }

    private static func localRouteConflictScore(
        routes: [EdgeIdentifier: EdgeRoute],
        affectedEdges: [CompoundGraph.CardEdge],
        baseRouteSegmentIndex: RouteSegmentIndex,
        ignoringIndexedEdgeIDs: Set<EdgeIdentifier> = []
        ...
        for edge in affectedEdges {
            ...
            let score = edgeRouteConflictScore(
                routePoints(from: route),
                currentEdge: edge,
                routeSegmentIndex: baseRouteSegmentIndex,
                ignoring: ignoringIndexedEdgeIDs
            )
        ...
        for lhsIndex in 0..<(affectedEdges.count - 1) {
            ...
            guard let lhsRoute = routes[lhsEdge.id] else { continue }
            let lhsPoints = routePoints(from: lhsRoute)
            for rhsIndex in (lhsIndex + 1)..<affectedEdges.count {
                let rhsEdge = affectedEdges[rhsIndex]
                guard let rhsRoute = routes[rhsEdge.id] else { continue }
                let pairScore = routePairConflictScore(
                    lhsEdge: lhsEdge,
                    lhsPoints: lhsPoints,
                    rhsEdge: rhsEdge,
    ...
    private static func routeSegmentConflictScore(
        ...
        let sharedEndpoints = sharedEndpointFlags(lhsEdge, rhs.edge)
        guard
            let lhsSegment = edgeClearanceSegment(
                start: lhsStart,
                end: lhsEnd,
                sourceEndpoint: sharedEndpoints.lhsSource ? lhsPoints.first : nil,
                targetEndpoint: sharedEndpoints.lhsTarget ? lhsPoints.last : nil
            ),
            let rhsSegment = edgeClearanceSegment(
                start: rhs.start,
                end: rhs.end,
                sourceEndpoint: sharedEndpoints.rhsSource ? rhs.points.first : nil,
        ...
        guard distance < LayoutSpacing.edgeEdgeRoute else {
            return KnowledgeGraphRouteConflictScore(crossings: crossings, clearancePenalty: 0)
        }
    ...
    private struct FixedEndpointRouteKey: Hashable {
        let sourceX: Int
        let sourceY: Int
        let sourceSide: EdgePortSide
        let targetX: Int
        let targetY: Int
        let targetSide: EdgePortSide
        let laneOffset: Int
        let sourceIndex: Int
        let targetIndex: Int
        ...
        init(
            ...
            sourceX = Self.quantize(sourcePort.point.x)
            sourceY = Self.quantize(sourcePort.point.y)
            sourceSide = sourcePort.side
            targetX = Self.quantize(targetPort.point.x)
            targetY = Self.quantize(targetPort.point.y)
            targetSide = targetPort.side
            self.laneOffset = Self.quantize(laneOffset)
    ...
    private struct FixedEndpointRouteCandidateCache {
        private var storage: [FixedEndpointRouteKey: [[CGPoint]]] = [:]

        mutating func candidates(
            for key: FixedEndpointRouteKey,
            build: () -> [[CGPoint]]
        ) -> [[CGPoint]] {
    ...
    private static func orthogonalEdgePoints(
        ...
        func bestRoute(portPairs: [(source: EdgePort, target: EdgePort)]) -> [CGPoint]? {
            var best: [CGPoint]?
            var bestScore: KnowledgeGraphRouteCost?

            func ensureBestClearancePenalty() {
                guard
                    let best,
                    let score = bestScore,
                    score.clearancePenalty < 0
                else { return }
                bestScore = KnowledgeGraphRouteCost(
                    length: score.length,
                    ...
                    portCenterPenalty: score.portCenterPenalty
                )
            }
            ...
            func routeScore(
                ...
                sourcePort: EdgePort,
                targetPort: EdgePort
            ) -> KnowledgeGraphRouteCost {
                let needsClearancePenalty: Bool
                if let bestScore {
                    needsClearancePenalty = abs(metrics.length - bestScore.length) <= 0.001
                        && metrics.corners == bestScore.corners
                } else {
                    needsClearancePenalty = false
                }
                if needsClearancePenalty {
                    ensureBestClearancePenalty()
                }
                let clearancePenalty = needsClearancePenalty
                    ...
                    : -1
                return KnowledgeGraphRouteCost(
            ...
            for (sourcePort, targetPort) in portPairs {
                if let directPoints = directFixedEndpointRoute(
                    ...
                    targetPort: targetPort,
                    nodeIndex: nodeIndex,
                    excludingFirst: sourceIndex,
                    excludingSecond: targetIndex
                ) {
                ...
                for points in candidates {
                    ...
                    guard routeJointsClearEndpointNodes(
                        ...
                        firstEndpoint: sourceIndex,
                        secondEndpoint: targetIndex
                    ) else { continue }
    ...
    private static func shortestFixedEndpointRoute(
        ...
        let candidates = routeCandidateCache.candidates(for: key) {
            orthogonalRouteCandidates(
                ...
                points.count > 1
                    && simplifiedRouteIsOrthogonal(points)
                    && routeUsesPerpendicularPorts(points, sourcePort: sourcePort, targetPort: targetPort)
                    && routeClearsNodes(points, nodeIndex: nodeIndex, excluding: excluded)
                    && routeJointsClearEndpointNodes(
                        points,
                        cardRects: nodeIndex.cardRects,
                        endpointIndices: excluded
        ...
        for points in candidates {
            ...
            let score = KnowledgeGraphRouteCost(
                length: metrics.length,
                corners: metrics.corners,
                clearancePenalty: clearancePenalty,
                portCenterPenalty: 0
    ...
    private static func directFixedEndpointRoute(
        ...
        guard routeClearsNodes(
            points,
            nodeIndex: nodeIndex,
            excludingFirst: first,
            excludingSecond: second
        ) else {
            return nil
        }
        guard routeJointsClearEndpointNodes(
    ...
    private static func orthogonalRouteCandidates(
        ...
        let usesCompactCandidates = nodeIndex.cardRects.count > RoutingTuning.compactCandidateNodeCount
        let stubDistances = usesCompactCandidates
            ? uniqueCGFloatValues([
                0,
        ...
        let laneOffsets = usesCompactCandidates
            ? uniqueCGFloatValues([
                laneOffset,
                0,
                -laneOffset
        ...
        var obstacleSeeds: [ObstacleRouteSeed] = []
        obstacleSeeds.reserveCapacity(stubDistances.count * stubDistances.count)
        var hasClearingSimpleCandidate = false
            ...
            for targetDistance in stubDistances {
                ...
                if !usesCompactCandidates {
                    ...
                    if simpleCandidateClearsNodeConstraints {
                        hasClearingSimpleCandidate = true
                    } else {
                        obstacleSeeds.append(ObstacleRouteSeed(
                            sourceOut: sourceOut,
                            targetOut: targetOut,
        ...
        let shouldUseObstacleSearch = !usesCompactCandidates && (!hasClearingSimpleCandidate
            || nodeIndex.cardRects.count <= RoutingTuning.exactObstacleRoutingNodeCount
        )
        if shouldUseObstacleSearch {
            let seedLimit = hasClearingSimpleCandidate
                ? RoutingTuning.maxSupplementalObstacleSeeds
                : RoutingTuning.maxRequiredObstacleSeeds
            for seed in obstacleSeeds.sorted(by: { $0.isPreferred(to: $1) }).prefix(seedLimit) {
                guard let obstacleAvoidingCore = obstacleAvoidingOrthogonalRoute(
                    start: seed.sourceOut,
                    end: seed.targetOut,
                    laneOffset: laneOffset,
                    nodeIndex: nodeIndex,
                    excludedIndices: excludedIndices
                ) else { continue }
                let route = routeWithEndpointStubs(
                    sourcePoint: sourcePort.point,
                    sourceOut: seed.sourceOut,
                    core: obstacleAvoidingCore,
                    targetOut: seed.targetOut,
                    targetPoint: targetPort.point
                )
                guard routeClearsNodes(route, nodeIndex: nodeIndex, excluding: excludedIndices) else {
                    continue
                }
                guard routeJointsClearEndpointNodes(
                    route,
    ...
    private struct ObstacleRouteSeed {
        let sourceOut: CGPoint
        let targetOut: CGPoint
        let sourceDistance: CGFloat
        let targetDistance: CGFloat

        func isPreferred(to other: ObstacleRouteSeed) -> Bool {
            let distance = sourceDistance + targetDistance
            let otherDistance = other.sourceDistance + other.targetDistance
            if abs(distance - otherDistance) > 0.001 {
                return distance < otherDistance
            }
            let length = abs(sourceOut.x - targetOut.x) + abs(sourceOut.y - targetOut.y)
            let otherLength = abs(other.sourceOut.x - other.targetOut.x)
                + abs(other.sourceOut.y - other.targetOut.y)
            if abs(length - otherLength) > 0.001 {
                return length < otherLength
            }
            return sourceDistance < other.sourceDistance
        }
    }
    ...
    private static func obstacleAvoidingOrthogonalRoute(
        ...
        let rawObstacles = nodeIndex.nodes(in: searchRect).compactMap { node -> CGRect? in
            excludedIndices.contains(node.index) ? nil : node.rect
        }
        let obstacles = boundedObstacleRects(rawObstacles, start: start, end: end)
        guard !obstacles.isEmpty else { return nil }
        if let detour = deterministicObstacleAvoidingOrthogonalRoute(
            start: start,
            end: end,
            laneOffset: laneOffset,
            obstacles: obstacles
        ...
        guard nodeIndex.cardRects.count <= RoutingTuning.exactObstacleRoutingNodeCount else {
            return nil
        }
        ...
        guard width * height * 3 <= RoutingTuning.maxRouteSearchStates else {
            return nil
        }
        ...
        while let entry = queue.popMin() {
            ...
            expansions += 1
            guard expansions <= RoutingTuning.maxRouteSearchExpansions else { return nil }
    ...
    private static func boundedObstacleRects(
        _ rects: [CGRect],
        start: CGPoint,
        end: CGPoint
    ) -> [CGRect] {
        guard rects.count > RoutingTuning.maxObstacleRects else { return rects }
        let midpoint = CGPoint(x: (start.x + end.x) * 0.5, y: (start.y + end.y) * 0.5)
        return Array(rects.sorted { lhs, rhs in
            let leftScore = obstacleRouteRelevanceScore(lhs, start: start, end: end, midpoint: midpoint)
            let rightScore = obstacleRouteRelevanceScore(rhs, start: start, end: end, midpoint: midpoint)
            if abs(leftScore - rightScore) > 0.001 {
                return leftScore < rightScore
            }
            return layoutArea(lhs) > layoutArea(rhs)
        }.prefix(RoutingTuning.maxObstacleRects))
    }

    private static func obstacleRouteRelevanceScore(
        _ rect: CGRect,
        start: CGPoint,
        end: CGPoint,
        midpoint: CGPoint
        ...
        return pointSegmentDistance(center, start, end)
            + pointRectDistance(midpoint, rect) * 0.25
            + min(pointRectDistance(start, rect), pointRectDistance(end, rect)) * 0.1
    }

    private static func deterministicObstacleAvoidingOrthogonalRoute(
        start: CGPoint,
        end: CGPoint,
        laneOffset: CGFloat,
        obstacles: [CGRect]
        ...
        var xValues = [start.x, end.x, midX]
        var yValues = [start.y, end.y, midY]
        for obstacle in obstacles {
            xValues.append(obstacle.minX - lanePadding)
            xValues.append(obstacle.maxX + lanePadding)
            yValues.append(obstacle.minY - lanePadding)
        ...
        xValues = limitedDetourValues(xValues, target: midX)
        yValues = limitedDetourValues(yValues, target: midY)

        var best: [CGPoint]?
        var bestMetrics: RouteMetrics?
        var seen: Set<RouteCandidateKey> = []

        func consider(_ rawPoints: [CGPoint]) {
            let points = simplifyRoutePoints(rawPoints)
            guard points.count > 1 else { return }
            let key = RouteCandidateKey(points: points.map(QuantizedRoutePoint.init))
            guard seen.insert(key).inserted else { return }
            guard simplifiedRouteIsOrthogonal(points) else { return }
            guard routeClearsObstacles(points, obstacles: obstacles) else { return }
            let metrics = simplifiedRouteMetrics(points)
            if let current = bestMetrics {
                if metrics.length < current.length - 0.001 ||
                    (abs(metrics.length - current.length) <= 0.001 && metrics.corners < current.corners) {
                    best = points
                    bestMetrics = metrics
                }
            } else {
        ...
        consider([start, end])
        consider([start, CGPoint(x: end.x, y: start.y), end])
        consider([start, CGPoint(x: start.x, y: end.y), end])
        for x in xValues {
            consider([
                start,
        ...
        let compactX = Array(xValues.prefix(RoutingTuning.maxCompoundDetourAxisValues))
        let compactY = Array(yValues.prefix(RoutingTuning.maxCompoundDetourAxisValues))
        for x in compactX {
            for y in compactY {
                consider([
                    start,
    ...
    private static func limitedDetourValues(_ values: [CGFloat], target: CGFloat) -> [CGFloat] {
        let unique = sortedUniqueCGFloatValues(values)
        guard unique.count > RoutingTuning.maxDetourAxisValues else { return unique }
        return Array(unique.sorted { lhs, rhs in
            ...
            return lhs < rhs
        }.prefix(RoutingTuning.maxDetourAxisValues))
    }

    private static func routeClearsObstacles(_ points: [CGPoint], obstacles: [CGRect]) -> Bool {
        guard points.count > 1 else { return true }
        for offset in 1..<points.count {
            guard segmentClearsObstacles(points[offset - 1], points[offset], obstacles: obstacles) else {
                return false
            }
        }
        return true
    ...
    private static func uniqueCGFloatValues(_ values: [CGFloat]) -> [CGFloat] {
        ...
        var accepted: [(offset: Int, value: CGFloat)] = []
        accepted.reserveCapacity(values.count)
        var lastValue: CGFloat?
        for item in sorted {
            if let lastValue, abs(item.element - lastValue) < 0.5 {
                continue
            }
            accepted.append((offset: item.offset, value: item.element))
            lastValue = item.element
        }
        return accepted
    ...
    private static func routeJointsClearEndpointNodes(
        ...
        let simplified = simplifyRoutePoints(points)
        guard simplified.count > 2 else { return true }
        guard firstEndpoint < cardRects.count, secondEndpoint < cardRects.count else {
            return true
        }
        let firstRect = cardRects[firstEndpoint]
        let secondRect = cardRects[secondEndpoint]
        for jointIndex in 1..<(simplified.count - 1) {
            let joint = simplified[jointIndex]
            if pointRectDistance(joint, firstRect) < LayoutSpacing.jointNode - 0.001 {
                return false
            }
            if pointRectDistance(joint, secondRect) < LayoutSpacing.jointNode - 0.001 {
                return false
            }
        }
        return true
    ...
    private static func edgeRouteConflictScore(
        ...
        currentEdge: CompoundGraph.CardEdge,
        routeSegmentIndex: RouteSegmentIndex,
        ignoring ignoredEdgeIDs: Set<EdgeIdentifier> = []
    ) -> KnowledgeGraphRouteConflictScore {
        guard points.count > 1 else {
            return KnowledgeGraphRouteConflictScore(crossings: 0, clearancePenalty: 0)
        }
        ...
        for currentOffset in 1..<points.count {
            ...
            routeSegmentIndex.forEachSegmentNear(
                ...
                guard !ignoredEdgeIDs.contains(other.edge.id) else { return true }
    ...
    private static func sharedEndpointFlags(
        _ lhs: CompoundGraph.CardEdge,
        _ rhs: CompoundGraph.CardEdge
    ) -> SharedEndpointFlags {
        SharedEndpointFlags(
            lhsSource: lhs.source == rhs.source || lhs.source == rhs.target,
            lhsTarget: lhs.target == rhs.source || lhs.target == rhs.target,
            rhsSource: rhs.source == lhs.source || rhs.source == lhs.target,
            rhsTarget: rhs.target == lhs.source || rhs.target == lhs.target
        )
    }
    ...
    private static func edgeClearanceSegment(
        ...
        sourceEndpoint: CGPoint?,
        targetEndpoint: CGPoint?
    ) -> (start: CGPoint, end: CGPoint)? {
        var trimmedStart = start
        var trimmedEnd = end
        if let sourceEndpoint, pointsAreClose(trimmedStart, sourceEndpoint) {
        ...
        if let targetEndpoint, pointsAreClose(trimmedEnd, targetEndpoint) {
    ...
    private struct IndexedEdgeLabelRect {
        let edgeID: EdgeIdentifier
        let rect: CGRect
    }
    ...
    private static func placeEdgeLabels(
        ...
        let cardIndex = edgeLabelCardIndex(cardRects: cardRects)
        let blockingRouteSegmentIndex = edgeLabelBlockingRouteSegmentIndex(routes: routes)
        let placementPasses = candidateSets.count > EdgeLabelTuning.largeGraphEdgeCount
            ? EdgeLabelTuning.largeGraphPlacementPasses
            : EdgeLabelTuning.regularPlacementPasses
        for _ in 0..<placementPasses {
            var labelIndex = edgeLabelRectIndex(labelRects: placedLabelRects)
            for candidateSet in candidateSets {
                let center = bestEdgeLabelPosition(
                    ...
                    blockingRouteSegmentIndex: blockingRouteSegmentIndex
                )
                placed[candidateSet.edge.id] = center
                let rect = edgeLabelRect(center: center, size: candidateSet.size)
                placedLabelRects[candidateSet.edge.id] = rect
                labelIndex.removeAll { $0.edgeID == candidateSet.edge.id }
                labelIndex.insert(
                    IndexedEdgeLabelRect(edgeID: candidateSet.edge.id, rect: rect),
                    rect: rect
                )
            }
        }
    ...
    private static func bestEdgeLabelPosition(
        ...
        size: CGSize,
        cardIndex: SpatialHashIndex<CGRect>,
        labelIndex: SpatialHashIndex<IndexedEdgeLabelRect>,
        blockingRouteSegmentIndex: SpatialHashIndex<EdgeRouteSegment>
    ) -> CGPoint {
    ...
    private static func edgeLabelCardIndex(cardRects: [CGRect]) -> SpatialHashIndex<CGRect> {
        var index = SpatialHashIndex<CGRect>()
    ...
    private static func edgeLabelRectIndex(
        labelRects: [EdgeIdentifier: CGRect]
    ) -> SpatialHashIndex<IndexedEdgeLabelRect> {
        var index = SpatialHashIndex<IndexedEdgeLabelRect>()
        for (edgeID, rect) in labelRects {
            index.insert(IndexedEdgeLabelRect(edgeID: edgeID, rect: rect), rect: rect)
        }
        return index
    }

    private static func edgeLabelBlockingRouteSegmentIndex(
        routes: [EdgeIdentifier: EdgeRoute]
    ) -> SpatialHashIndex<EdgeRouteSegment> {
        var index = SpatialHashIndex<EdgeRouteSegment>()
        for (edgeID, route) in routes {
            let points = routePoints(from: route)
            guard points.count > 1 else { continue }
            for offset in 1..<points.count {
                let segment = EdgeRouteSegment(
                    edgeID: edgeID,
    ...
    private static func edgeLabelAnchorSamples(_ route: EdgeRoute, labelSize: CGSize) -> [RoutePathSample] {
        ...
        let points = routePoints(from: route)
        if points.count >= 4 {
            return limitedEdgeLabelSamples(
                edgeLabelSecondSegmentSamples(points, labelSize: labelSize)
        ...
        return limitedEdgeLabelSamples(samples)
    }
    ...
    private static func edgeLabelSamplesOnSegment(
        ...
        var offset = step
        var pairCount = 0
        let maxPairCount = max(0, (EdgeLabelTuning.maxSamplesPerSegment - 1) / 2)
        while offset <= length * 0.5 - endGuard + 0.5, pairCount < maxPairCount {
    ...
    private static func limitedEdgeLabelSamples(_ samples: [RoutePathSample]) -> [RoutePathSample] {
        guard samples.count > EdgeLabelTuning.maxSamplesPerEdge else { return samples }
        return Array(samples.prefix(EdgeLabelTuning.maxSamplesPerEdge))
    }
    ...
    private static func edgeLabelCollisionMetrics(
        ...
        let cardQueryRect = rect.insetBy(
            dx: -LayoutSpacing.edgeNode,
            dy: -LayoutSpacing.edgeNode
        )
        cardIndex.forEach(in: cardQueryRect) { entry in
            let card = entry.item
        ...
        let labelQueryRect = rect.insetBy(
            dx: -LayoutSpacing.edgeLabelLabel,
            dy: -LayoutSpacing.edgeLabelLabel
        )
        labelIndex.forEach(in: labelQueryRect) { entry in
            guard entry.item.edgeID != edgeID else { return true }
            let occupied = entry.item.rect.insetBy(
                dx: -LayoutSpacing.edgeLabelLabel,
                dy: -LayoutSpacing.edgeLabelLabel
            )
        ...
        blockingRouteSegmentIndex.forEach(in: routeRect) { entry in
            let segment = entry.item
            if segment.edgeID != edgeID,
               segmentIntersectsRect(segment.start, segment.end, routeRect) {
